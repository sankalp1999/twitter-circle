	...
	try {
		...
		if (textExists || title === 'Profile / X' || title === 'Log in to X / X' || currentUrl.includes('redirect_after_login')) {
			...
			if (title === 'Log in to X / X' || currentUrl.includes('redirect_after_login')) {
				return { twitterUsername: twitterUsername, profileJsonLd: 'redirect_case', weight: weight}
			}
			return { twitterUsername: twitterUsername, profileJsonLd: 'dne', weight: weight }
		}
		...
		return { twitterUsername: twitterUsername, profileJsonLd: jsonLdContent, weight: weight }
	} catch (error) {
		console.error(`Error fetching JSON-LD content for ${twitterUsername}: ${error.message}`)
		return { twitterUsername: twitterUsername, profileJsonLd: null, weight: weight }
	} finally {
		await page.close()
	}
}
...
(async () => {
	try {
		...
		usernames = usernames.slice(0, 400)
